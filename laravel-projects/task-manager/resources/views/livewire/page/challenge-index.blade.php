<div class="max-w-5xl mx-auto py-8 space-y-8">
  @if(session()->has('message'))
    <div class="p-3 rounded bg-blue-100 text-blue-800 dark:bg-blue-900 dark:text-blue-200">{{ session('message') }}</div>
  @endif
  <div class="bg-white dark:bg-gray-800 shadow rounded-lg p-6">
    ...
    <form wire:submit.prevent="create" class="grid sm:grid-cols-2 gap-4">
      <div>
        ...
        <input type="text" wire:model="title" placeholder="100 Days of Code" class="w-full" />
      </div>
      <div>
      ...
      <div>
        ...
        <input type="number" min="1" max="365" wire:model="target_days" class="w-full" />
      </div>
  ...
  <div class="grid md:grid-cols-2 gap-6">
      ...
      <ul class="space-y-2">
        @forelse($owned as $run)
          <li class="flex justify-between items-center">
            <div>
              <div class="font-medium">{{ $run->title ?? '100 Days of Code' }}</div>
              <div class="text-xs text-muted-foreground">Début: {{ $run->start_date->format('Y-m-d') }} • {{ $run->target_days }} jours</div>
            </div>
            <x-ui.button :link="true" href="{{ route('challenges.show', $run) }}" title="Ouvrir" />
          </li>
        @empty
    ...
    <div class="bg-white dark:bg-gray-800 shadow rounded-lg p-6">
      ...
      <ul class="space-y-2">
        ...
        @empty
          <li class="text-sm text-muted-foreground">Aucun challenge rejoint.</li>
        @endforelse
      </ul>
    </div>
  </div>
