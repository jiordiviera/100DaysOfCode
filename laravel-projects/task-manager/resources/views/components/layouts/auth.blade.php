...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}" class="scroll-smooth">
  ...
  <body class="font-sans antialiased min-h-screen !overflow-x-hidden bg-background text-foreground">
    <main class="grid lg:grid-cols-2 w-full min-h-screen">
      <!-- Colonne image (50%) -->
      <aside class="hidden lg:block relative">
        <img src="{{ $image ?? '/images/auth-illustration.svg' }}" alt="{{ $imageAlt ?? 'Illustration' }}" class="absolute inset-0 w-full h-full object-cover" />
        <div class="absolute inset-0 bg-gradient-to-b from-background/10 to-background/30"></div>
        @if(!empty($heroTitle) || !empty($heroSubtitle))
          <div class="absolute bottom-0 left-0 right-0 p-10">
            ...
            @endisset
            @isset($heroSubtitle)
              <p class="text-sm text-muted-foreground mt-2 max-w-md">{{ $heroSubtitle }}</p>
            @endisset
          </div>
        @endif
      </aside>
